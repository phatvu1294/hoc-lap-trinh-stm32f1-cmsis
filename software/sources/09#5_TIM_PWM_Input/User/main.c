...
int main(void)
{
  ...
  /* TIM3 SlaveReset Mode */
  TIM3->SMCR &= ~TIM_SMCR_TS; // Clear
  TIM3->SMCR |= TIM_SMCR_TS_2 | TIM_SMCR_TS_0; // 101: Filtered Timer Input 1 Trigger
  TIM3->SMCR &= ~TIM_SMCR_SMS; // Clear
  TIM3->SMCR |= TIM_SMCR_SMS_2; // 100: Reset Mode

  /* Cấu hình TIM3 Input Capture Channel 1 Direct (Period)  */
  ...
  /* Cấu hình TIM3 Input Capture Channel 2 Indirect (Duty)  */
  TIM3->CCMR1 &= ~TIM_CCMR1_IC2F; // 0000: No filter, sampling is done at fDTS
  ...
  TIM3->CCMR1 |= TIM_CCMR1_CC2S_1; // 10: CC2 channel is configured as input, IC1 is mapped on TI2
  TIM3->CCER |= TIM_CCER_CC2P; // 1: inverted: capture is done on a "falling edge" of IC1. When used as external trigger, IC1 is inverted
  ...
  while (1)
  {
    /* Gửi giá trị tần số và độ rộng xung ra usart1 */
...
void TIM3_IRQHandler(void)
{
  ...
  if (((TIM3->SR & TIM_SR_CC1IF) == TIM_SR_CC1IF) && ((TIM3->DIER & TIM_DIER_CC1IE) == TIM_DIER_CC1IE))
  {
    ...
    icValue1 = TIM3->CCR1 + 1;

    /* Nếu giá trị khác 0 */
    if (icValue1 != 0)
    {
      /* Đọc giá trị Capture Duty */
      icValue2 = TIM3->CCR2 + 1;
